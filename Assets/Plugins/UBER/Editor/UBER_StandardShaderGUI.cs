...
namespace UnityEditor
{
	internal class UBER_StandardShaderGUI : ShaderGUI
	{
        ...
        */
		...
		static string alt_shader_snow = ""; // snow/no snow shader
		...
		private void CheckForAltShader(Material material, bool _Tessellation_Used, bool _Snow_Used) {
			...
			alt_shader_snow = ""; // snow / no snow shader
			if (_Snow_Used) {
				alt_shader_snow=material.shader.name.Replace("- DynSnow - ", "- ");
				if (!Shader.Find(alt_shader_snow)) alt_shader_snow="";
			} else {
				alt_shader_snow=material.shader.name.Replace("UBER - ", "UBER - DynSnow - ");
				if (!Shader.Find(alt_shader_snow)) alt_shader_snow="";
			}
			if (material.HasProperty("_Snow")) material.SetFloat ("_Snow", _Snow_Used ? 1:0);
			#if UNITY_5_0
			if (!PlayerSettings.useDirect3D11) alt_shader = "";
			#else
			#endif
		...
		public void ShaderPropertiesGUI (Material material, MaterialProperty[] props)
		{
			...
			bool _Snow_Used=false, _Snow_hasMixedValue=false, _Snow_hasAllOff=false;
			GetStaticProp(material, "SNOW", ref _Snow_Used, ref _Snow_hasMixedValue, ref _Snow_hasAllOff);
			if (!_Snow.hasMixedValue) {
				if (!_Snow_Used && _Snow.floatValue == 1) _Snow.floatValue = 0;
				else if (_Snow_Used && _Snow.floatValue == 0) _Snow.floatValue = 1;
			}
			
			if (!alt_shader_inited) {
				CheckForAltShader(material, _Tessellation_Used, _Snow_Used);
            ...
            bool _AlbedoOcclusion_Used = occlusionMap.textureValue == null || _UVSecOcclusion.floatValue==1 || _FORCE_DIFFAO_Used;
...
//				}
                ...
                */
				...
				if (_MainShown.floatValue == 1 || _MainShown.hasMixedValue) {
					DoAlbedoArea (material, _AlbedoOcclusion_Used, _TwoLayers_Used);
					...
					if (_AlbedoOcclusion_Used) {
                        ...
                        {
                            EditorGUILayout.HelpBox("With forced AO from diffuse (checked in shader tag) occlusion map acts only for coloring mask", MessageType.Warning);
                        }
                    }
                    if (_TwoLayers_Used) {
				...
				
				{
					...
					float nval;
					EditorGUI.BeginChangeCheck ();
					EditorGUI.BeginDisabledGroup(!alt_shader_inited || alt_shader_snow=="");
					...
					if (EditorGUI.EndChangeCheck ()) {
						if (m_MaterialEditor!=null) {
							foreach(Material mat in m_MaterialEditor.targets) {
								string tmp_alt_shader_snow="";
								bool _SnowTmp_Used=false, _SnowTmp_hasMixedValue=false, _SnowTmp_hasAllOff=false;
								GetStaticProp(mat, "SNOW", ref _SnowTmp_Used, ref _SnowTmp_hasMixedValue, ref _SnowTmp_hasAllOff);
								if (_SnowTmp_Used && nval==0) {
									tmp_alt_shader_snow=mat.shader.name.Replace("- DynSnow - ", "- ");
									if (!Shader.Find(tmp_alt_shader_snow)) tmp_alt_shader_snow="";
								} else if (!_SnowTmp_Used && nval==1) {
									tmp_alt_shader_snow=mat.shader.name.Replace("UBER - ", "UBER - DynSnow - ");
									if (!Shader.Find(tmp_alt_shader_snow)) tmp_alt_shader_snow="";
								}
								if (tmp_alt_shader_snow!="") {
									mat.shader=Shader.Find(tmp_alt_shader_snow);
									if (mat.HasProperty("_Snow")) mat.SetFloat ("_Snow", nval);
								}
							}
						}
					}
		...
		void DoAlbedoArea(Material material, bool _AlbedoOcclusion_Used, bool _TwoLayers_Used)
		{
			
			m_MaterialEditor.TexturePropertySingleLine(_AlbedoOcclusion_Used ? Styles.albedoOccText : Styles.albedoText, albedoMap, albedoColor);
			if (albedoMap.textureValue != null && _AlbedoOcclusion_Used && (BlendMode)blendMode.floatValue==BlendMode.Opaque) {
				ShaderPropertyTooltip( occlusionStrength, Styles.occlusionStrengthText, 2);
			}
			if (_TwoLayers_Used) {
				m_MaterialEditor.TexturePropertySingleLine (_AlbedoOcclusion_Used ? new GUIContent(Styles.albedoOccText.text+" 2", Styles.albedoOccText.tooltip) : new GUIContent(Styles.albedoText.text+" 2", Styles.albedoText.tooltip), albedoMap2, albedoColor2);
				if (albedoMap2.textureValue != null && _AlbedoOcclusion_Used && (BlendMode)blendMode.floatValue == BlendMode.Opaque) {
		...
		void DoEmissionArea(Material material, bool _Animated_Emission)
		{
			...
			m_MaterialEditor.TexturePropertyWithHDRColor(_Animated_Emission ? Styles.emissionAnimatedText : Styles.emissionText, emissionMap, emissionColorForRendering, m_ColorPickerHDRConfig, false);
		...
		{
			if (m_WorkflowMode == WorkflowMode.Specular) {
				if (specularMap.textureValue == null && (specularMap2.textureValue == null || !_TwoLayers_Used) && _SpecularRGBGlossADetail.textureValue == null) { // (gloss ranged also for detail spec map)
					EditorGUI.BeginChangeCheck();
				...
				if (metallicMap.textureValue == null && (metallicMap2.textureValue == null || !_TwoLayers_Used) && _MetallicGlossMapDetail.textureValue == null) {
					_GlossMin.floatValue=0; // explicit gloss (neither from main nor detail texture)
					_GlossMax.floatValue=1;
				} else {
					ShaderPropertyTooltip(_GlossMin, Styles._GlossMinText,2);
		...
		static void SetMaterialKeywords(Material material, WorkflowMode workflowMode)
		{
			...
			bool _AlbedoOcclusion_Used = (material.GetTexture("_OcclusionMap") == null || material.GetFloat("_UVSecOcclusion") == 1 || material.GetTag("FORCE_DIFFAO", false) == "On");
			SetKeyword (material, "_OCCLUSION_FROM_ALBEDO_ALPHA", _AlbedoOcclusion_Used );
		...
		void FixSubstanceTexelSizeBug(MaterialProperty[] props) {
			...
			*/
			Debug.Log("Hack by akamis. Need to use External Substance Tool");
		}
	}
}